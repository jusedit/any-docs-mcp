...
export interface Section {
  ...
  title: string;
  level: number;
  path: string[];  // hierarchical path of parent titles
  content: string;
...
export class MarkdownParser {
  ...
  public search(query: string, options: { 
    ...
    const scored = sections.map(section => {
      ...
      // Boost for code blocks if searching for code-related terms
  ...
  public getOverview(): string {
    const index = this.getIndex();
    const lines: string[] = ['# Shopware Documentation Overview\n'];
